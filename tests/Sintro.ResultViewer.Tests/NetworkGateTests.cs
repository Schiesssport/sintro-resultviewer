...
/// <summary>
/// Boots the API with an allowlist that deliberately excludes the test client's source
/// address, so the gate's refusal path is exercised for real rather than mocked.
/// </summary>
...
public class NetworkGateTests : IClassFixture<GatedApiFixture>
{
    ...
    {
        // With a valid token the answer must still be 403, never 401 or 200: a blocked
        // network never gets as far as presenting credentials.
...
public class TrustedProxyTests(TrustedProxyApiFixture fixture) : IClassFixture<TrustedProxyApiFixture>
{
    ...
    {
        // client 10.1.2.50 → proxy 172.20.0.1 (trusted) → us. Both hops are ours, so the
        // original client is believable and passes the allowlist.
        var client = fixture.CreateClient();
    ...
    public async Task unwindingStopsAtTheFirstHopWeDoNotTrust()
    {
        ...
        // believed — it becomes the client, and it is not on the allowlist.
        var client = fixture.CreateClient();
